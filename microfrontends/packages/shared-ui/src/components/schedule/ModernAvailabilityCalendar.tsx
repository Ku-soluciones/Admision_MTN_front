...
const ModernAvailabilityCalendar: React.FC<ModernAvailabilityCalendarProps> = ({
  ...
  const getSummaryStats = useCallback(() => {
    ...
    days.forEach(day => {
      ...
      if (selectedSlots.length > 0) {
        ...
        for (let i = 1; i < selectedSlots.length; i++) {
          ...
          if (diffMinutes > 60) {
            // End current range
            const endTime = new Date(prevTime.getTime() + 60 * 60000);
            ranges.push({
    ...
    const totalHours = (totalBlocks).toFixed(1);
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-blue-100 mt-1">
              Define cuándo puedes recibir entrevistas durante la semana. Cada bloque equivale a 60 minutos.
            </p>
          </div>
