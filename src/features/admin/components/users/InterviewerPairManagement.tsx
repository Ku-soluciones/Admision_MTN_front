  ...
  return (
    <section aria-labelledby="pair-management-title" className="space-y-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <h2 id="pair-management-title" className="text-lg font-bold text-gray-950">Parejas de entrevistas</h2>
          <p className="mt-1 max-w-3xl text-sm text-gray-600">
            Vincula un Director de Ciclo con un Psicólogo y define los niveles que pueden atender juntos.
          </p>
        </div>
      ...
      {formOpen && (
        <form onSubmit={submit} className="rounded-xl border border-gray-300 bg-gray-50 p-4" aria-label={editing ? 'Editar pareja' : 'Crear pareja'}>
          <div className="flex items-start justify-between gap-4">
            <div>
              <h3 className="font-bold text-gray-950">{editing ? 'Editar niveles o integrantes' : 'Nueva pareja por niveles'}</h3>
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-2">
            <label className="text-sm font-semibold text-gray-800">Director de Ciclo
              <select required value={directorId} onChange={(event) => setDirectorId(event.target.value)} className="mt-1 min-h-11 w-full rounded-lg border border-gray-300 bg-white px-3 text-gray-950 focus:border-[#008a57] focus:outline-none focus:ring-2 focus:ring-[#008a57]/30">
                <option value="">Seleccionar Director de Ciclo</option>
          ...
          <fieldset className="mt-5">
            ...
            <div className="mt-1 flex flex-wrap items-center justify-between gap-2">
              ...
              <div className="flex gap-2">
                <button type="button" onClick={() => setGrades(options.grades.map((grade) => grade.code))} className="min-h-9 rounded-lg px-3 text-xs font-semibold text-[#006b45] hover:bg-emerald-50 focus:outline-none focus:ring-2 focus:ring-[#008a57]">Seleccionar todos</button>
                <button type="button" onClick={() => setGrades([])} disabled={grades.length === 0} className="min-h-9 rounded-lg px-3 text-xs font-semibold text-gray-700 hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-[#008a57] disabled:cursor-not-allowed disabled:opacity-50">Limpiar</button>
              </div>
            </div>
            <div className="mt-2 flex flex-wrap gap-2">
              {options.grades.map((grade) => {
                const selected = grades.includes(grade.code);
                return <label key={grade.code} className={`inline-flex min-h-11 cursor-pointer items-center gap-2 rounded-lg border px-3 py-2 text-sm font-medium focus-within:ring-2 focus-within:ring-[#008a57] ${selected ? 'border-[#008a57] bg-emerald-50 text-emerald-950' : 'border-gray-300 bg-white text-gray-700'}`}>
                  <input type="checkbox" checked={selected} onChange={() => toggleGrade(grade.code)} className="h-4 w-4 accent-[#008a57]" />{grade.label}
                </label>;
              })}
            </div>
          </fieldset>
          <div className="mt-5 flex flex-wrap justify-end gap-2">
            ...
            <button type="submit" disabled={saving || availableDirectors.length === 0 || availablePsychologists.length === 0} className="inline-flex min-h-11 items-center gap-2 rounded-lg bg-[#008a57] px-4 text-sm font-semibold text-white hover:bg-[#007247] focus:outline-none focus:ring-2 focus:ring-[#008a57] focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-60">
              {saving && <FiRefreshCw className="animate-spin" aria-hidden="true" />}{editing ? 'Guardar nueva revisión' : 'Crear pareja'}
            </button>
          </div>
        </form>
      )}
      ...
      {loading ? (
        <div className="flex min-h-32 items-center justify-center gap-2 text-sm text-gray-700" role="status"><FiRefreshCw className="animate-spin" aria-hidden="true" /> Cargando parejas…</div>
      ) : pairs.length === 0 ? (
        <div className="rounded-xl border border-gray-300 bg-gray-50 p-8 text-center">
          ...
          <h3 className="mt-3 font-bold text-gray-950">Aún no hay parejas configuradas</h3>
        ...
        <div className="overflow-x-auto rounded-xl border border-gray-300">
          <table className="min-w-full divide-y divide-gray-300 text-left text-sm">
            ...
            <tbody className="divide-y divide-gray-200 bg-white">
              {pairs.map((pair) => <tr key={pair.id} className={!pair.active ? 'bg-gray-50 text-gray-600' : ''}>
                <td className="px-4 py-4"><p className="font-semibold text-gray-950">{pair.cycleDirector.name}</p><p className="text-gray-600">Director de Ciclo</p><p className="mt-2 font-semibold text-gray-950">{pair.psychologist.name}</p><p className="text-gray-600">Psicólogo/a</p></td>
                <td className="max-w-md px-4 py-4"><div className="flex flex-wrap gap-1.5">{pair.grades.map((grade) => <span key={grade} className="rounded-full bg-gray-100 px-2.5 py-1 text-xs font-semibold text-gray-700">{gradeLabels.get(grade) || grade}</span>)}</div></td>
                <td className="px-4 py-4"><span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-bold ${pair.active ? 'bg-emerald-100 text-emerald-900' : 'bg-gray-200 text-gray-700'}`}>{pair.active ? `Activa · revisión ${pair.revision}` : `Archivada · revisión ${pair.revision}`}</span></td>
                <td className="px-4 py-4"><div className="flex justify-end gap-2">{pair.active && <><button type="button" onClick={() => openEdit(pair)} className="inline-flex min-h-11 items-center gap-2 rounded-lg border border-gray-300 px-3 font-semibold text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-[#008a57]"><FiEdit2 aria-hidden="true" /> Editar</button><button type="button" onClick={() => archive(pair)} className="inline-flex min-h-11 items-center gap-2 rounded-lg border border-gray-300 px-3 font-semibold text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-[#008a57]"><FiArchive aria-hidden="true" /> Archivar</button></>}</div></td>
              </tr>)}
            </tbody>
          </table>
        </div>
