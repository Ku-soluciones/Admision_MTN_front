...
const criteria = [
  { title: "Separación del adulto", description: "Cómo se separa el postulante del adulto que lo acompaña al ingreso." },
  { title: "Contacto visual", description: "Establece contacto visual espontáneo con el evaluador." },
  { title: "Disposición inicial", description: "Muestra disposición y apertura al comenzar la jornada." },
  { title: "Respuesta al nombre", description: "Responde cuando se le llama por su nombre." },
  { title: "Exploración del espacio", description: "Explora el entorno de manera autónoma y curiosa." },
  { title: "Regulación durante el ingreso", description: "Mantiene la regulación emocional durante el ingreso." },
] as const;
...
export function ConnectedIndicatorsConsole({ profile }: Props) {
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-4 rounded-xl border border-teal-200 bg-teal-50 p-4 text-teal-950">
        <div className="flex items-center gap-3">
          <UserCheck size={20} />
          <div>
            <p className="text-sm font-black">Espacio exclusivo: Evaluador de Indicadores de Ingreso</p>
            <p className="text-xs text-teal-800">{assignments.length} asignaciones para hoy</p>
          </div>
        </div>
        <button
          onClick={() => void loadAgenda()}
          className="rounded-lg bg-white px-3 py-2 text-xs font-black text-teal-800 hover:bg-teal-100"
        >
          Actualizar
        </button>
      </div>
            ...
            <div className="grid gap-4 p-5 md:grid-cols-3">
              {selectedAssignment.reports.map((report, index) => (
                <article key={report.applicationId} className="relative rounded-xl border border-slate-200 p-5 text-center">
                  ...
                  <span className="mx-auto grid h-14 w-14 place-items-center rounded-full bg-teal-50 font-black text-teal-900">
        ...
        <div>
          <div className="mb-5 flex flex-wrap items-end justify-between gap-3">
            <div>
              ...
              <h2 className="mt-1 text-3xl font-black text-slate-950">Evaluación de Indicadores de Ingreso</h2>
              <p className="mt-1 text-sm text-slate-600">Un criterio a la vez, con los tres postulantes visibles en paralelo.</p>
            </div>
            <span className="rounded-xl bg-teal-50 px-4 py-3 text-sm font-black text-teal-900">
          ...
          <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_12px_32px_rgba(15,23,42,0.05)]">
            <div className="flex flex-wrap items-center gap-4 border-b border-slate-200 p-5">
              <span className="grid h-14 w-14 place-items-center rounded-xl bg-teal-900 text-xl font-black text-white">{criterionIndex + 1}</span>
            ...
            <div className="grid gap-4 p-5 xl:grid-cols-3">
              {members.map((report) => {
                const options = [
                  ...
                  { value: 1 as Score, title: "1", label: "En desarrollo inicial" },
                  { value: 2 as Score, title: "2", label: "En desarrollo medio" },
                  { value: 3 as Score, title: "3", label: "Desarrollado" },
                  { value: 4 as Score, title: "4", label: "Completamente desarrollado" },
                ...
                return (
                  <article key={report.applicationId} className="rounded-xl border border-slate-200 p-4">
                    <div className="flex items-center gap-3 border-b border-slate-200 pb-4">
                      <span className="grid h-11 w-11 place-items-center rounded-full bg-teal-50 text-sm font-black text-teal-900">
                        {report.applicantName.split(" ").slice(0, 2).map((p) => p[0] ?? "").join("")}
                      </span>
                      <div>
                    ...
                    <div className="mt-4 grid grid-cols-2 gap-2">
                      {options.map((opt) => (
                        <button
                          ...
                          className={`min-h-14 rounded-lg border p-2 transition ${currentResponses[report.applicationId] === opt.value ? "border-2 border-teal-700 bg-teal-50 text-teal-950" : "border-slate-200 bg-white hover:border-teal-300"}`}
