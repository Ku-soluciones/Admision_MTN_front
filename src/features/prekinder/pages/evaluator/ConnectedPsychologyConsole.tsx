...
export function ConnectedPsychologyConsole({ profile }: Props) {
  ...
  return (
    <div className="space-y-5">
      {/* Header */}
      <div className="flex flex-wrap items-center justify-between gap-4 rounded-xl border border-blue-200 bg-gradient-to-r from-[#1e3a8a] to-[#1e4a9a] p-4 text-white">
        <div className="flex items-center gap-3">
          <UserCheck size={20} />
          <div>
            <p className="text-sm font-black">Evaluación de Psicología</p>
          ...
          <section className="rounded-2xl border border-slate-200 bg-white">
            <div className="grid gap-4 p-5 md:grid-cols-3">
              {selectedAssignment.reports.map((report, index) => (
                ...
                >
                  <span className="absolute left-3 top-3 grid h-7 w-7 place-items-center rounded-full bg-[#1e3a8a] text-xs font-black text-white">
                    {index + 1}
                  </span>
                  <span className="mx-auto grid h-16 w-16 place-items-center rounded-full bg-blue-100 text-xl font-black uppercase text-[#1e3a8a]">
        ...
        <div>
          ...
          {isCurrentBlocked && minutesUntilStart !== null && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm">
              <div className="mx-4 max-w-sm rounded-2xl border border-blue-200 bg-gradient-to-br from-[#1e3a8a] to-[#1e4a9a] p-8 text-center text-white shadow-2xl">
                ...
                <button
                  onClick={() => navigate("/prekinder/evaluador/psychology")}
          ...
          <div className="grid gap-5 xl:grid-cols-[270px_1fr]">
            ...
            <aside className="h-fit rounded-2xl border border-slate-200 bg-gradient-to-b from-[#1e3a8a] to-[#1e4a9a] p-4">
                    ...
                    >
                      ...
                      <span className="min-w-0 flex-1">
                        <b
                          className={`block truncate text-sm font-bold leading-tight ${
                            isActive && isEditable ? "text-[#1e3a8a]" : "text-white"
                          }`}
                        >
                        ...
                        {!isEditable ? (
                          ...
                          <span className="mt-0.5 flex items-center gap-1 text-xs font-bold text-blue-200">
                            <Check size={11} />
                            Completo
                          </span>
                        ) : completion > 0 ? (
                      ...
                      {isEditable &&
                        (personComplete ? (
                          ...
                          <ChevronRight
                            size={16}
                            className={`flex-shrink-0 ${isActive ? "text-[#1e3a8a]" : "text-white/60"}`}
                          />
                        ))}
                    </button>
                  );
            ...
            <section className="rounded-2xl border border-slate-200 bg-white shadow-lg overflow-hidden">
              ...
              <div className="rounded-t-2xl border-b border-slate-200 bg-gradient-to-r from-[#1e3a8a] to-[#1e4a9a] p-6">
                <div className="flex items-center gap-5">
                  <span
                    className="grid h-20 w-20 place-items-center rounded-full bg-white text-2xl font-black uppercase tracking-wide text-[#1e3a8a]"
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-1 text-sm font-medium text-blue-200">
                      Psicología · Evaluación Individual
                    </p>
                  </div>
                ...
                {rubricCriteria.map((criterion, cIdx) => (
                  <div key={criterion.criterionId} className="p-5">
                    <div className="mb-4 flex items-start gap-3">
                      <span className="grid h-8 w-8 flex-shrink-0 place-items-center rounded-lg bg-[#1e3a8a] text-sm font-black text-white">
                        {cIdx + 1}
                      </span>
                      <div className="min-w-0 flex-1">
                        <p
