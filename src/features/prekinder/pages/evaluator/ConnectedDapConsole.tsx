...
export function ConnectedDapConsole({ profile }: Props) {
  ...
  return (
    <div className="space-y-5">
      {/* Header */}
      <div className="flex flex-wrap items-center justify-between gap-4 rounded-xl border border-violet-200 bg-gradient-to-r from-violet-800 to-violet-900 p-4 text-white">
        ...
        <button
          ...
          className="rounded-lg bg-white px-3 py-2 text-xs font-black text-violet-900 hover:bg-violet-50"
        >
          Actualizar
        </button>
      </div>
          ...
          <section className="rounded-2xl border border-slate-200 bg-white">
            <div className="grid gap-4 p-5 md:grid-cols-3">
              {selectedAssignment.reports.map((report, index) => (
                ...
                >
                  <span className="absolute left-3 top-3 grid h-7 w-7 place-items-center rounded-full bg-violet-900 text-xs font-black text-white">
                    {index + 1}
                  </span>
        ...
        <div>
          ...
          {isCurrentBlocked && minutesUntilStart !== null && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm">
              <div className="mx-4 max-w-sm rounded-2xl border border-violet-200 bg-gradient-to-br from-violet-800 to-violet-900 p-8 text-center text-white shadow-2xl">
                ...
                <button
                  onClick={() => navigate("/prekinder/evaluador/dap")}
          ...
          <div className="grid gap-5 xl:grid-cols-[270px_1fr]">
            ...
            <aside className="h-fit rounded-2xl border border-slate-200 bg-gradient-to-b from-violet-800 to-violet-900 p-4">
              <h3 className="mb-3 px-2 text-xs font-black uppercase tracking-widest text-violet-200">
                Postulantes
              </h3>
            ...
            <section className="rounded-2xl border border-slate-200 bg-white shadow-lg overflow-hidden">
              ...
              <div className="rounded-t-2xl border-b border-slate-200 bg-gradient-to-r from-violet-800 to-violet-900 p-6">
                <div className="flex items-center gap-5">
                  <span
                    className="grid h-20 w-20 place-items-center rounded-full bg-white text-2xl font-black uppercase tracking-wide text-violet-900"
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-1 text-sm font-medium text-white/80">
                      DAP · Evaluación Individual · Acceso Restringido
                    </p>
                  </div>
                ...
                {rubricCriteria.map((criterion, cIdx) => (
                  <div key={criterion.criterionId} className="p-5">
                    <div className="mb-4 flex items-start gap-3">
                      <span className="grid h-8 w-8 flex-shrink-0 place-items-center rounded-lg bg-violet-900 text-sm font-black text-white">
                        {cIdx + 1}
                      </span>
                      <div className="min-w-0 flex-1">
                        <p
                              ...
                              isSelected && isLogrado
                                ...
                                : isSelected && isPorLograr
                                  ...
                                  : isSelected && isNoLogrado
                                    ...
                                    : isSelected
                                      ? "border-violet-700 bg-violet-900 text-white shadow-md"
                                      : "border-slate-200 bg-white hover:border-violet-400 hover:shadow-sm disabled:bg-slate-50 disabled:text-slate-400"
                            }`}
                          >
