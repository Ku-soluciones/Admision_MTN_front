...
- **Development Commands** — All available npm commands
- **Architecture Overview** — How the project is structured
- **Common Workflows** — Step-by-step guides for common tasks
- **Troubleshooting** — Debug common issues
- **Known Issues** — Code duplication and future improvements

## Quick Start

```bash
npm install                    # Install all dependencies (monorepo)
npm run dev:mf:admin          # Start admin dashboard on http://localhost:5206
# Or run all 9 apps:
npm run dev                    # http://localhost:5200 (shell), 5201-5208 (others)
```

**Key Points:**
- Always access via `http://localhost:52xx` (not `file:///`)
- Ports are strictly enforced—if a port is busy, Vite will fail explicitly
- Each microfrontend runs independently; code changes hot-reload via Vite HMR
- Env vars from root `.env.development` apply to all apps

## Technology Stack

| Layer | Technology | Version | Notes |
|-------|-----------|---------|-------|
| **UI Framework** | React | 19.1.0 | Functional components with hooks |
| **Routing** | React Router | 7.6.3 | Hash-based (`/#/...`) routing |
| **Build Tool** | Vite | 6.2.0 | Per-app config in `vite.config.ts` |
| **HTTP Client** | Axios | 1.11.0 | With request interceptors for auth/CSRF |
| **State Query** | TanStack React Query | 5.90.2 | For API state management |
| **Date Handling** | date-fns | 4.1.0 | Immutable date utilities |
| **Charts** | ECharts + echarts-for-react | 6.0.0 / 3.0.6 | Data visualization |
| **Icons** | Heroicons, Lucide React | 2.2.0 / 0.540.0 | Icon libraries |
| **File Upload** | react-dropzone | 14.3.8 | Drag-and-drop uploads |
| **Excel Export** | xlsx | 0.18.5 | Spreadsheet generation |
| **Auth** | Firebase + OIDC | 12.12.1 / oidc-client-ts 3.3.0 | Multi-auth support |
| **Testing** | Playwright | 1.48.0 | E2E & integration tests |
| **Language** | TypeScript | 5.7.2 | Strict type checking |

## Development Commands

```bash
# All microfrontends
npm run dev                    # Start all 9 apps (shell + 8 microfrontends)
npm run build                  # Build all apps to dist/

# Individual microfrontends (fastest for focused work)
npm run dev:mf:shell           # Shell/gateway (5200)
...
# Type checking
npx tsc --noEmit               # Check TypeScript errors (entire monorepo)
cd microfrontends/apps/mf-xxx
npx tsc --noEmit               # Check single app

# Testing
npx playwright install         # Install browser binaries (one-time)
...
- `microfrontends/packages/shared-ui`: Shared components, services, utilities
...
```json
"workspaces": ["microfrontends/apps/*", "microfrontends/packages/*"]
```

**This means:**
- Each app (`mf-admin`, `mf-admissions`, etc.) has its own `package.json` and runs independently
- Each package (`backend-sdk`, `shared-ui`, etc.) is shared—changes here affect all apps that import from it
- `npm install` installs dependencies for all workspaces at once (no need to install per-app)
- Apps do NOT share component code at runtime, only types and utilities from `packages/`

**When working on a package:**
1. Edit code in `microfrontends/packages/shared-ui/src/`
2. Apps that import from it will see changes via symlink (in development)
3. Test in multiple apps before considering it "shared"

**When working on an app:**
1. Edit code in `microfrontends/apps/mf-xxx/`
2. Run that app locally with `npm run dev:mf:xxx`
3. Changes are isolated—won't affect other apps until deployed

## Environment Configuration

**Files (root directory):**
- `.env.development` — Shared by all apps during `npm run dev`
- `.env.staging`, `.env.production` — Environment-specific vars
...
- Non-VITE variables can be server-only (but frontend has no server access in SPA mode)

**Key variables:**
- `VITE_API_BASE_URL`: Backend gateway (used by api.ts interceptor)
- `VITE_APP_ENV`: Environment name for URL building (dev, staging, production)
- `VITE_MF_BASE_DOMAIN`: Base domain for URL resolution (e.g., `admitia.dedyn.io`)
- `VITE_MF_ENV`: Environment prefix (production, staging, dev, uat)
- `VITE_MF_*_URL`: Per-microfrontend URL overrides (if a domain doesn't follow the convention)

**Example:** With `VITE_MF_BASE_DOMAIN=admitia.dedyn.io` and `VITE_MF_ENV=staging`:
- `mf-admissions` → `https://admision.staging.admitia.dedyn.io`
- `mf-guardian` → `https://guard.staging.admitia.dedyn.io`
- `mf-admin` → `https://admin.staging.admitia.dedyn.io`

URLs are auto-constructed; no hardcoding needed.
...
**Code Sharing:**
- **No shared component imports at runtime:** If you need to reuse UI code across apps, move it to a shared package under `microfrontends/packages/` and import it, or duplicate (if very small). Avoid importing from one app's `src/` into another app.

**Environment & Configuration:**
- **Don't commit `.env` files:** Environment files in root are git-ignored. Local overrides belong in `.env.local` (also ignored). Team coordination needed for `.env.development`.
- **Never put secrets in `VITE_*` variables:** These are embedded in the browser bundle. Use server-only env vars if needed, but this is a frontend SPA—the backend handles secrets.
...
**Storage & Persistence:**
- **localStorage keys must use `getStorageKey(BASE_STORAGE_KEYS.*)`:** Raw string keys will collide across apps and environments. Always use the backend-sdk wrapper.
...
- **Never commit sensitive data:** API keys, tokens, or credentials in code will be visible to anyone with repo access.

**Git Workflow:**
- **Branch naming:** Use `feature/darkmork/feature-name` or `fix/darkmork/bug-name` (include your username)
- **Commit messages:** Use semantic convention:
  ```bash
  git commit -m "feat(mf-admin): add user role filter to admin table"
  git commit -m "fix(mf-evaluations): handle missing student data in enrichment"
  ```
- **Before pushing:** Run TypeScript check on the microfrontend you changed:
  ```bash
...
npm run dev:mf:admin  # Restart the specific app
```

**Module not found / import errors:**
- Check that aliases in `vite.config.ts` match your import (e.g., `@components` points to `src/components/`)
- Verify the file actually exists (path is case-sensitive on macOS/Linux)
- If importing from packages, ensure the dependency is listed in both app's and package's `package.json`
- Run TypeScript check to confirm: `npx tsc --noEmit`

**Blank page or HMR not working:**
- Verify you're accessing via `http://localhost:52xx`, not `file:///`
- Check browser console for errors (press F12)
- Restart the dev server if HMR stops responding
...
- Check that key uses `getStorageKey(BASE_STORAGE_KEYS.AUTH_TOKEN)` pattern (never raw strings)
- Open DevTools → Application → Local Storage to inspect actual keys
- Verify browser allows localStorage (check privacy settings, third-party cookies)

**Blank page after login redirect:**
- Verify the target microfrontend is running on the expected port (5200-5208)
- Check that `microfrontendUrls.ts` resolves to the correct URL
- Open DevTools Network tab for CORS or fetch errors
- Check if the target app requires additional setup (e.g., Firebase config)

**User data missing in AuthContext:**
- Verify BFF endpoint `/v1/auth/check` returns complete user profile
- Check localStorage for the actual data: `localStorage.getItem(getStorageKey(BASE_STORAGE_KEYS.AUTHENTICATED_USER))`
- If missing, the auth flow didn't complete—check Network tab for `/v1/auth/check` response

### API & Network

**API calls fail with 400 or 401:**
- Ensure auth token is in localStorage: `localStorage.getItem(getStorageKey(BASE_STORAGE_KEYS.AUTH_TOKEN))`
- Check Network tab: `Authorization` header should be `Bearer <token>`
- Verify token is not expired (backend validates TTL)
- If 401, verify user has the required role (check `user.role` in console)
- Check if request URL is in `isPublicRoute()` list in `services/api.ts`

**API calls fail with 403 (Forbidden):**
- Backend rejected the request due to insufficient permissions
- Verify user has the required role for this endpoint
- Check backend logs for the specific reason

**ERROR 500 on API endpoint:**
- This is a backend issue, not frontend
- Check backend logs for the actual error
- Verify request payload matches backend schema (use Network tab to inspect)
- For domain-specific errors (evaluator assignment, document upload), check if all required fields are present

**CORS errors in browser console:**
- Typically means API base URL is wrong or backend doesn't allow requests from this origin
- Verify `VITE_API_BASE_URL` in `.env.development`
- Check if backend is running and accessible
- Verify backend CORS headers allow requests from localhost

### Data & Display Issues

**UI shows placeholder text instead of actual data:**
- Follow the "Debugging Data Issues" section in "Common Workflows"
- Check DevTools Network tab to see the actual API response
- Add `console.log()` statements in the service to trace data flow
- Verify data enrichment is working (for nested objects)

**Email not sending (no error shown):**
- Check if the email service method is actually being called (add `console.log`)
- Verify you're using the correct endpoint: `/v1/institutional-emails/...` or `/v1/email/...`
- Check backend has SMTP credentials configured
- Verify email template exists and has correct placeholders
- See `.gstack/qa-reports/EMAIL_FLOWS_AUDIT_2026-05-09.md` for which flows are partially implemented

**Tables or lists showing old data:**
- Check React Query cache: Open DevTools → React Query tab (if dev tools installed)
- Force refetch: Look for a "Refresh" button or clear cache manually: `localStorage.clear()`
- Verify the API actually returned new data (check Network tab)

### Cross-App Issues

**Unexpected behavior from other apps:**
- Check if other apps are running on their expected ports (5200-5208)
- Verify `microfrontendUrls.ts` resolves to correct URLs for your environment
- Remember: Apps are isolated—code changes in one don't affect others until deployed
- If testing cross-app flow, run `npm run dev` to start all apps simultaneously

**Navigation between apps fails:**
- Check browser console for errors when clicking cross-app link
- Verify the target URL is correct by opening it directly in the address bar
- Check if the target app's dev server is actually running
- If using custom domains, verify DNS resolves correctly

## Type Checking & Testing

### Type Checking

TypeScript is configured but **not enforced in the build pipeline**. Check errors manually:
...
**Best practice:** Run type checking before committing:
```bash
# Before pushing a PR, verify your changes have no type errors
cd microfrontends/apps/mf-admin
npx tsc --noEmit
```
...
Playwright is installed (`@playwright/test` in package.json) but not yet integrated into the build pipeline.
...
**Create a test file:**
Create `tests/example.spec.ts` in a microfrontend:
```typescript
import { test, expect } from '@playwright/test';

test('login flow', async ({ page }) => {
  await page.goto('http://localhost:5206/#/login');
...
**Future Integration:** Consider adding `playwright test` to a pre-commit hook or CI/CD pipeline to catch UI regressions early.
