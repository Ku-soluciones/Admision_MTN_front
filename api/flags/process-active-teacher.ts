...
/**
 * GET /api/flags/process-active-teacher
 *
 * Evalúa el flag `process-active-teacher`.
 *
...
export default async function handler(_req: Request) {
  try {
    const flag = await resolveFlagValue('process-active-teacher');
    return jsonResponse(flag);
  } catch (error) {
    console.error('[Flags] Error evaluating process-active-teacher:', error);
